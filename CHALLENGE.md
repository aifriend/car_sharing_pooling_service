...
:warning: The challenge needs to be self-contained so we can evaluate it. Things like external databases will prevent the automated tests from executing successfully after the challenge is submitted, so consider using [GitLab's CI services](https://docs.gitlab.com/ee/ci/services/) instead.
